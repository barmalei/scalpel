...
import lev_distance 
import gravity.tae.distance
import gravity.tae.match.clev
import gravity.tae.text


class fLevDistance(lev_distance.fBaseLevDistance):
    def _distance(self, text, pattern):
        return gravity.tae.match.clev.fLevDistance(text, pattern)
        
    def generator(self, text, pattern):
        text, pattern = unicode(text), unicode(pattern)
        data = gravity.tae.match.clev.fLevDistanceMatrix(text, pattern)
    ...
    def matrix(self, text, pattern):
        class _UprightDistanceMatrix(gravity.tae.distance.BaseDistanceMatrix):
            def __init__(self, data, rows, cols):
                assert data and len(data) > 0 and len(data) == rows*cols
                gravity.tae.distance.BaseDistanceMatrix.__init__(self)
                self.rows = rows
                self.cols = cols
                self._data = data
        ...
        text, pattern = unicode(text), unicode(pattern)
        return _UprightDistanceMatrix(gravity.tae.match.clev.fLevDistanceMatrix(text, pattern), len(text), len(pattern))
...
class fLevDistanceDiag(lev_distance.fLevDistanceDiag):        
    ...
    def _calc(self, text, pattern, def_value = lev_distance.fBaseLevDistanceDiag.MAX_VALUE):
        d = gravity.tae.match.clev.fLevDistanceDiag(text, pattern, self._normalized_delta(len(text)), def_value)
        return d 
...
class fLevPath(lev_distance.fDistancePath):
    def path(self, matrix):
        ...
        res = gravity.tae.match.clev.fLevPath(matrix.data, matrix.rows, matrix.cols, (matrix.colsize - 1)/2)
        for r in res: yield r


class fLevTextsAligner(gravity.tae.text.fTextsAligner):
    ...
    def _align(self, original_text, processed_text, offsets):
        ...
        return gravity.tae.match.clev.fLevTextAlignment(processed_text, original_text, self.delta, offsets)        


class fLevTokensAligner(gravity.tae.text.fTokensAligner):
